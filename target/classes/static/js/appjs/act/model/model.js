				...
				columns : [
					...
					},
					{
						...
						formatter : function(value, row, index) {
							...
							var f = '<a class="btn btn-success btn-sm ' + s_resetPwd_h + '" href="#" title="Deploy process"  mce_href="#" onclick="deploy(\''
								+ row.id
...
function deploy(id) {
    layer.confirm('Confirm to deploy the selected model？', {
        btn : [ 'Confirm', 'Cancel' ]
    }, function() {
        $.ajax({
