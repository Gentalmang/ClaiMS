...
//					ActUtils.exportDiagramToFile(repositoryService, processDefinition, exportDir);
                ...
                message += "Successful deployment, process ID=" + processDefinition.getId() + "<br/>";
            }

            if (list.size() == 0) {
                message = "Deployment failed, no process.";
            }

        } catch (Exception e) {
            throw new ActivitiException("Deployment faile！", e);
        }
        return R.ok(message);
    }
    ...
    public R convertToModel(@PathVariable("procDefId") String procDefId, RedirectAttributes redirectAttributes) throws UnsupportedEncodingException, XMLStreamException {
        ...
        try {
            ...
            return R.ok( "Model conversion succeed, model ID=" + modelData.getId());
        } catch (Exception e) {
            e.printStackTrace();
            return R.ok( "Model conversion failed");
        }
