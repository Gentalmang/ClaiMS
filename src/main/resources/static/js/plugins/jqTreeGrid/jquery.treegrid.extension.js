(function($) {
    ...
    $.fn.bootstrapTreeTable = function(options, param) {
        ...
        target.load = function(parms){
            ...
            target.append(tbody);
            // 添加加载loading
            var _loading = '<tr><td colspan="'+options.columns.length+'"><div style="display: block;text-align: center;">Trying to load data, please wait……</div></td></tr>'
            tbody.html(_loading);
            // 默认高度
            if(options.height){
            ...
            $.ajax({
                ...
                success : function(data, textStatus, jqXHR) {
                    ...
                    if(!data||data.length<=0){
                        var _empty = '<tr><td colspan="'+options.columns.length+'"><div style="display: block;text-align: center;">No record</div></td></tr>'
                        tbody.html(_empty);
                        return;
                    }
